...
function determineSiteCategoryLabel(raw) {
    ...
    // 1. 食品・飲料・調味料
    if (/(食品|食材|調味料|だし|出汁|味噌|みそ|醤油|しょうゆ|お茶|緑茶|日本茶|コーヒー|紅茶|ほうじ茶|玄米茶|米|お菓子|スナック|乾物|漬物|海苔|のり|昆布|だしパック|だしの素)/.test(text) ||
    ...
    // 4. ケトル・コーヒー用品
    if (/(ケトル|やかん|ドリップケトル|コーヒーミル|コーヒーグラインダー|コーヒードリッパー|コーヒーサーバー|ドリッパー|コーヒーポット|ティーポット|急須)/.test(text) ||
        /(コーヒー|ティーウェア)/.test(category)) {
        return 'ケトル・コーヒー用品';
    }

    // 5. キッチンツール・調理小物
    if (/(まな板|カッティングボード|おろし器|ピーラー|キッチンバサミ|キッチンはさみ|キッチンスケール|はかり|計量スプーン|しゃもじ|菜箸|トング|泡立て器|キッチンタイマー|保存容器|キッチンツール)/.test(text) ||
    ...
    // 7. 食器・カトラリー
    if (/(食器|プレート|皿|大皿|小皿|ランチプレート|茶碗|ちゃわん|飯碗|汁椀|お椀|マグ|マグカップ|カップ|湯のみ|グラス|コップ|カトラリー|箸|お箸|スプーン|フォーク|ナイフセット)/.test(text) ||
        /(食器)/.test(category)) {
        return '食器・カトラリー';
    }

    // 8. キッチンマット・シンク周り
    if (/(キッチンマット|台所マット|シンクマット|シンク用マット|シンクラック|水切りラック|水切りかご|水切りカゴ|ディッシュラック|排水口カバー|排水口ネット|三角コーナー)/.test(text)) {
        return 'キッチンマット・シンク周り';
    }

    // 9. 生活雑貨・インテリア
    if (/(カーテン|のれん|インテリア|収納|ボックス|収納ケース|クッション|ラグ|マット|マルチカバー|玄関マット|スリッパ|エコバッグ|買い物袋|レジ袋|タオルハンガー|ティッシュケース)/.test(text) ||
        /(カーテン|インテリア|収納用品)/.test(category)) {
        return '生活雑貨・インテリア';
    }

    // 10. ヘルス・ビューティー・日用品（サプリ含む）
    if (/(石鹸|ソープ|シャンプー|リンス|コンディショナー|トリートメント|ローション|クリーム|乳液|マスク|フェイスマスク|入浴剤|バスソルト|タオル|フェイスタオル|バスタオル|爪切り|ネイルクリッパー|歯ブラシ|マウスウォッシュ)/.test(text) ||
    ...
    // 11. ペット用品
    if (/(ペット|犬用|猫用|ドッグフード|キャットフード|犬用おやつ|猫用おやつ|ペットフード|ペットシーツ|ペットベッド|首輪|リード|ペットブラシ|ペット用シャンプー)/.test(text) ||
        /(ペット用品|ペットグッズ|犬用品|猫用品)/.test(category)) {
        return 'ペット用品';
    }

    // 12. 服飾・アクセサリー
    if (/(リング|指輪|ネックレス|ブレスレット|バングル|ピアス|イヤリング|アクセサリー|帽子|ベルト|財布|ウォレット|ポーチ|ハンカチ)/.test(text) ||
    ...
    // 13. ホビー・クラフト
    if (/(ギター|ベース|楽器|ストラップ|ピック|ホビー|クラフト|手芸|ソーイング|ミシン糸|編み物|釣り|ルアー|PEライン)/.test(text) ||
