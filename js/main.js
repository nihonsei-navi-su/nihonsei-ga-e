...
function determineSiteCategoryLabel(raw) {
    ...
    // 8. 生活雑貨・インテリア（照明・カレンダーを追加）
    if (
        /(カーテン|のれん|インテリア|収納|ボックス|収納ケース|クッション|ラグ|マット|マルチカバー|玄関マット|スリッパ|エコバッグ|買い物袋|レジ袋|タオルハンガー|ティッシュケース|照明|ライト|シーリングライト|スタンドライト|デスクライト|ペンダントライト|間接照明|カレンダー|卓上カレンダー|壁掛けカレンダー)/.test(text) ||
    ...
    // 11. ファッション・アクセサリー（パンプス・靴を追加）
    if (
        /(指輪|ネックレス|ブレスレット|バングル|ピアス|イヤリング|アクセサリー|帽子|ベルト|財布|ウォレット|ポーチ|ハンカチ|靴|シューズ|パンプス|サンダル|ブーツ|ローファー)/.test(text) ||
